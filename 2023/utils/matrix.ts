...
export class Matrix<T> {
    ...
    getCell(x: number, y: number): MatrixCell<T> {
        return this.cells[y][x];
    }

    getCellFromCoords(coords: Coords): MatrixCell<T> {
        return this.cells[coords.y][coords.x];
    }

    getSurroundingCells(coords: Coords): MatrixCell<T>[] {
        ...
        for (let x = Math.max(coords.x - 1, 0); x <= Math.min(coords.x + 1, this.width - 1); x++) {
            for (let y = Math.max(coords.y - 1, 0); y <= Math.min(coords.y + 1, this.height - 1); y++) {
                ...
                if (!newCoords.equals(coords)) {
                    res.push(this.getCellFromCoords(newCoords));
                }
            }
        }
        return res;
...
export class MatrixCell<T> {
    ...
    constructor(coords: Coords, cell: T) {
        this.coords = coords;
        this.content = cell;
    }
}